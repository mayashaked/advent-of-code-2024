...
def part1():
    ...
    for rule_pair in rules:
        ...
        for i, update in enumerate(updates):
            if to_print_before in update and to_print_after in update:
                if update.index(to_print_before) > update.index(to_print_after):
                    if i not in faulty_updates:
                        faulty_updates.append(i)

    correct_updates_middle_sum = 0

    for i, update in enumerate(updates):
        if i not in faulty_updates:
            correct_updates_middle_sum += int(update[len(update)//2])
